  ...
  <div id="app" class="container">
    <nav class="navbar is-fixed-top" role="navigation" aria-label="main navigation">
      ...
      <div id="statusBar" class="navbar-menu">
        <a class="navbar-item id_button"  @click="takeName">
          <span  :style="'padding: 0.4rem; outline: 1px solid ' + colour">
    ...
    submitComment () {
      ...
      this.axios.post(url, this.newComment)
        .then((response) => {
          ...
          this.entries.unshift(response.data.data)
          this.$nextTick(() => this.$redrawVueMasonry('masonryContainer'))
        })
    },
    submitTask () {
      let url = '/v1/locations/' + this.newTask.location_id + '/tasks'
      this.newTask.username = this.$store.state.name
      this.newTask.colour = this.$store.state.colour
      this.newTask.user_location_id = this.$store.state.location
      this.newTask.translations_attributes = []
      this.availableLocales.forEach((l) => {
        if (this.newTask[l]) {
          this.newTask.translations_attributes.push({locale: l, summary: this.newTask[l] })
        }
      })
      this.axios.post(url, this.newTask)
        .then((response) => {
          this.prepareEntry(response.data.data.id)
          this.entries.unshift(response.data.data)
          this.$nextTick(() => this.$redrawVueMasonry('masonryContainer'))
